...
val_file   = os.path.join(root_dir, "val/out.csv")
...
class CityScapesDataset(Dataset):
    ...
    def __len__(self):
        return 2975 #len(self.data)

    def __getitem__(self, idx):
        img_name   =  root_dir + '/train/img/' + self.data.img[idx]
        label_name = root_dir + '/train/label/' + self.data.label[idx]
        #img        = io.imread(img_name, mode='RGB')    
        img        = mpimg.imread(img_name)*255.0 # Reads in RGB format
        label      = mpimg.imread(label_name)*255.0
